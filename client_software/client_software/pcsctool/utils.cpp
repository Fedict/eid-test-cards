	...
	const char *chipnrForCompare, int idx)
{
	...
	if (strcmp("80:E4:00:00:0A", cmdApdu) == 0)
	{
		// First 10 byte sof Get Card Data (= first 10 bytes of chipnr): only check SW12
		if (respApduLen != buf2Len || respApdu[buf2Len - 2] != buf2[buf2Len - 2] || respApdu[buf2Len - 1] != buf2[buf2Len - 1])
		{
			printf("ERR: SW12 to \"%s\"\n", cmdApdu);
			dumphex("  - virt. card: ", buf2, buf2Len);
			dumphex("  - real card:  ", respApdu, respApduLen);
			return 1;
		}
	}
	else if (respApduLen != buf2Len || memcmp(respApdu, buf2, respApduLen) != 0)
	{
