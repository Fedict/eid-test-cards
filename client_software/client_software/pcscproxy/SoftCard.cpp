...
SoftCard::SoftCard(SoftReader *softReader, SCARDHANDLE hardhandle, SCARDHANDLE softhandle) :
	itsHardHandle(hardhandle), itsSoftHandle(softhandle), itsSoftReader(softReader),
	chipnr(NULL), versionnumbers(NULL), fs(NULL), responseBuffer(NULL)
{
	responseBuffer = new ArrayBuffer<BYTE>();
	itsTransaction = new Transaction((unsigned long) softhandle);
	lastCmdToCard  = false;
}
...
SoftReader *SoftCard::getSoftReader() const
{
	return itsSoftReader;
}
...
LONG SoftCard::beginTransaction()
{
	itsTransaction->aquire();
	return SCARD_S_SUCCESS;
}

LONG SoftCard::endTransaction(DWORD dwDisposition)
{
...
bool SoftCard::firstTimeInTransaction() const
{
	return itsTransaction->justStarted();
}

void SoftCard::setLastCmdToCard(bool lastCmdToCard)
{
	this->lastCmdToCard = lastCmdToCard;
}

bool SoftCard::lastCmdWasToCard() const
{
	return lastCmdToCard;
}

LONG SoftCard::transmit(const APDURequest &req, OUT LPBYTE pbRecvBuffer, IN OUT LPDWORD pcbRecvLength) const
{
	itsTransaction->aquire();
	LONG result;
