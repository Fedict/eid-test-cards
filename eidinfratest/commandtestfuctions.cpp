	...
	OUT LPSCARDHANDLE phCard, OUT LPDWORD pdwActiveProtocol)
{
	...
	//zetes::pcscproxy::SoftReader *sr = srmngr->getSoftReaderByName(szReader, zetes::pcscproxy::N_SOFT);
	//		SoftReader *sr = new SoftReader(*iter, softname);

	std::string                  hardname = "ACS ACR38U 0";
	std::string                  softname = "!Virtual ACS ACR38U 0";
	zetes::pcscproxy::SoftReader *sr      = new SoftReader(hardname, softname);
	...
	IN OUT LPSCARD_IO_REQUEST pioRecvPci, OUT LPBYTE pbRecvBuffer, IN OUT LPDWORD pcbRecvLength)
{
	...
	zetes::pcscproxy::SoftCard *sc = sr->createSoftCard(1234);
	if (sc == NULL)
		printf("ERR: SoftReader::createSoftCard() failed\n");

	if (sr)
	{
		const zetes::pcscproxy::APDURequest apduReq = zetes::pcscproxy::APDURequest::createAPDURequest(pbSendBuffer, cbSendLength);
