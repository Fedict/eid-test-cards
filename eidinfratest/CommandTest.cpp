...
TEST(Transmit)
{
	...
	fedict::pcscproxy::SoftCard *sc = sr->createSoftCard(1234);
	...
	for (int i = 0; fixedTestAPDUS2[i] != NULL; i++)
	{
		...
		const fedict::pcscproxy::APDURequest apduReq     = fedict::pcscproxy::APDURequest::createAPDURequest(sendBuf, (DWORD) commandLength);
		DWORD                               recvBufLen2 = (DWORD) sizeof(recvBuf);;
		result2 = sc->transmit(apduReq, recvBuf, &recvBufLen2);
