...
TEST(Readers)
{
	...
	const fedict::pcscproxy::APDURequest apduReq = fedict::pcscproxy::APDURequest::createAPDURequest(data, bytes);
...
DWORD WINAPI SoftReaderThread1(LPVOID p)
{
	...
	const fedict::pcscproxy::APDURequest apduReq = fedict::pcscproxy::APDURequest::createAPDURequest(sendBuf, sendBufLen);

	Sleep(((ThreadParams *) p)->sleepValue);
	sc->transmit(apduReq, recvBuf, &recvBufLen);
