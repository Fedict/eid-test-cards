...
VirtualCard::VirtualCard(const char *path)
{
    ...
    // strcpy_s() 'crashes' if content is too long; and since this can be the case
    // (i.e. if you put a long friendly name) we have to cut it off ourselves.
	FindElement(m_file, "friendlyName", content, MAX_CONTENT);
    size_t fnLen = strlen(content);
    if (fnLen > 31)
        fnLen = 31;
	memcpy(m_friendlyName, content, fnLen);
    m_friendlyName[fnLen] = '\0';
